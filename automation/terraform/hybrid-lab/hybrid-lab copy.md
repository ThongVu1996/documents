...
Trước khi đi vào code, hãy nắm rõ luồng thực thi:

```
  Máy local                 HCP Cloud                Proxmox (Nội bộ)
    │                        │                          │
    │── terraform apply ────►│                          │
    │                        │── Giao lệnh cho Agent ──►│
    │                        │                          │── Gọi Proxmox API
...
Tiến hành login vào HCP cloud. Với token lấy ở mục (#### 2.2.4 Cách 2: Tạo Bằng API (Tự Động Hóa))

```bash
terraform login
```
Điền token hcp vào đây
...
Kiểm tra Tailscale sau khi VM được tạo:
...
2. Chạy lệnh: `tailscale ssh username@ip-vm-trong-tailscale` hoặc `tailscale ssh username@ten-goi-trong-tailscale` (xem ảnh ở mục 3.5.3 để thấy)
3. Xác thực qua trình duyệt (SSO) → Vào thẳng VM. Nếu sử dụng flag `ssh` rồi thì không cần bước này nữa.
...
### 4.1 Luồng Bàn Giao Cho Dev (Zero-Touch Onboarding)

Khi một Developer cần quyền tạo VM trên Proxmox, Admin chỉ cần thực hiện **một việc duy nhất**:

```
Admin                                           Developer
  │                                                 │
  │── Tạo tài khoản HCP + thêm vào Organization ──► │
  │   (hoặc cấp Team Token cho workspace cụ thể)    │
  │                                                 │
  │                                          terraform login
  │                                          terraform init
  │                                          terraform plan
...
Từ thời điểm đó, toàn bộ quá trình là **hoàn toàn tự động**. Dev không cần — và **không thể** — tiếp cận bất kỳ thông tin hạ tầng nào.
...
```
Developer A                  HCP Terraform              Proxmox
     │                             │                        │
     │  terraform apply            │                        │
     │  -var="owner=dev-alice"  ──►│                        │
     │  -var="ttl_hours=8"         │── Agent nhận lệnh ────►│
     │                             │                        │── Tạo VM: dev-alice-env
     │                             │                        │── Cài: Node.js, MySQL, Redis
     │                             │                        │── Gán IP: 192.168.10.21
     │◄── Output: IP + SSH info ── │◄── Báo cáo xong ───────│
     │                             │                        │
     │  [8 tiếng sau]              │                        │
     │  terraform destroy ────────►│── Agent nhận lệnh ────►│
     │                             │                        │── Xóa VM, giải phóng tài nguyên
```

Mỗi dev có một Workspace riêng trên HCP với biến `owner` và `ttl_hours` được định nghĩa sẵn. Dev chỉ cần `apply` để có môi trường và `destroy` khi xong việc.

**Lợi ích đạt được:**

- Không còn xung đột tài nguyên giữa các dev
- Tài nguyên Proxmox được tái sử dụng hiệu quả — VM chỉ tồn tại khi cần
- Admin kiểm soát được tổng số VM đang chạy qua HCP dashboard
...
| Use Case | Tần suất tạo VM | Vòng đời VM | Người trigger |
| :--- | :--- | :--- | :--- |
| Dev Environment On-Demand | Hàng ngày | 4–12 giờ | Developer (tự phục vụ) |
| Isolated DB Per Branch | Mỗi lần tạo PR | Suốt vòng đời PR | GitHub Actions (tự động) |
| CI/CD Runner On-Demand | Mỗi build job | 15–60 phút | GitHub Actions (tự động) |
| Staging Environment Clone | Mỗi release cycle | 3–7 ngày | Release Manager |
| Security Pentest Sandbox | Theo lịch pentest | 1–3 ngày | Security Team |
